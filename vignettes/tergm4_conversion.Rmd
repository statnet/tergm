...
- The `offset.coef.form` and `offset.coef.diss` arguments (if present) should 
  be combined into a single `offset.coef` argument, schematically of the form 
  `offset.coef = c(offset.coef.form, offset.coef.diss)`, corresponding to the 
  order in which we have included the formation and dissolution formulas in the 
  combined formula.  Here, the CMLE example has no offsets, so this applies 
  only to the EGMME example where we have only a single dissolution offset to 
  begin with.  The result is that we replace
