...
set.seed(3)
dynfit<-tergm(g1 ~ Form(~edges + degree(1)) + offset(Diss(~edges)), targets="formation", estimate="EGMME", constraints="discordTNT"~., offset.coef=coef.diss,target.stats=target.stats[-3],verbose=TRUE,control=control.tergm(SA.plot.progress=do.plot,SA.restart.on.err=FALSE,init=c(-log(.95/.05),0, coef.diss)))
...
set.seed(5)
dynfit2<-tergm(g1 ~ Form(~edges + degree(1)) + Diss(~edges), targets=~edges+degree(1)+mean.age, estimate="EGMME", constraints="discordTNT"~., target.stats=target.stats,control=control.tergm(SA.plot.progress=do.plot,SA.plot.stats=TRUE))
