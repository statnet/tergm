#  File tests/testthat/test-term-Statistics.R in package tergm, part of the Statnet suite
...
test_that("the .Statistics term behaves appropriately", {
  ...
  s2 <- summary(~.Statistics(ff0, stats0), basis = nw)
  ...
  s2 <- summary(~.Statistics(ff2, I(c(stats2, 27L))), basis = nw)
  expect_identical(s1, s2)
  ...
  s2 <- summary(~.Statistics(ff3, stats3), basis = nw)
  expect_identical(s1, s2)
  ...
  sim22 <- simulate(nw ~ Form(~edges) + Diss(~edges),
                    ...
                    time.slices = 100,
                    dynamic = TRUE,
                    monitor = ~.Statistics(ff2,I(c(stats2, 27L))),
                    output = "stats")
  ...
  sim32 <- simulate(nw ~ Form(~edges) + Diss(~edges),
                    ...
                    time.slices = 100,
                    dynamic = TRUE,
                    monitor = ~.Statistics(ff3,stats3),
                    output = "stats")

  expect_identical(sim31, sim32)

  m01 <- ergm_model(ff0, nw = nw)
  m02 <- ergm_model(~.Statistics(ff0, stats0), nw = nw)
  ...
  m12 <- ergm_model(~.Statistics(ff1, stats1), nw = nw)
  ...
  m22 <- ergm_model(~.Statistics(ff2, I(c(stats2, 27L))), nw = nw)
  ...
  m32 <- ergm_model(~.Statistics(ff3, stats3), nw = nw)
