#  File tests/testthat/test-sim-gf-sum.R in package tergm, part of the Statnet
...
simtest <- function(S, edges, dur, n, dir = FALSE, bip = 0) {
  ...
  dc <- suppressWarnings(
    matrix(rnorm(n * n), if (bip) bip else n, if (bip) n - bip else n)
  )
  if (!dir && !bip) dc <- dc + t(dc)
  # Note that if nw1!=nw2, then
  # summary(nw1~edgecov(dc)+edgecov.ages(dc))!=
  #   summary(nw2~edgecov(dc)+edgecov.ages(dc))
  ...
  g1 %n% "lasttoggle" <- cbind(
    as.edgelist(g1),
    -rgeom(network.edgecount(g1), 1 / 4)
  )
  ...
  dynsim <- simulate(
    ...
    time.slices = S,
    verbose = TRUE,
    monitor = ~edgecov("dc") + edgecov.ages("dc"),
    dynamic = TRUE
  )
  ...
  dynsim3 <- simulate(
    dynsim2 ~ Form(~edges) + Persist(~edges),
    coef = c(coef.form, coef.diss),
    time.slices = S,
    verbose = TRUE,
    monitor = ~edgecov("dc") + edgecov.ages("dc"),
  ...
  gf1.stats <- as.matrix(
    tergm.godfather(
      dynsim3 ~ edgecov("dc") + edgecov.ages("dc"),
      start = 0,
      end = S * 3
    )
  )

  # Replay the sim using networkDynamic with no explicit time params
  # (should be using net.obs.period)
  gf1b.stats <- as.matrix(
    tergm.godfather(dynsim3 ~ edgecov("dc") + edgecov.ages("dc"))
  )

  # Replay the simulation using a network + list of changes:
  gf2.stats <- as.matrix(
    tergm.godfather(
      g1 ~ edgecov("dc") + edgecov.ages("dc"),
      start = 0,
      end = S * 3,
      changes = attr(dynsim3, "changes")
    )
  )
  ...
  expect_equal(sim.stats, gf1.stats)
  expect_equal(sim.stats, gf1b.stats)
  expect_equal(sim.stats, gf2.stats)
  expect_equal(sim.stats, summ.stats)
}

test_that("simulate, godfather, and summary statistics are consistent (undirected unipartite)", {
  set.seed(1)
  simtest(20, 10, 10, 10, FALSE, FALSE)
})

test_that("simulate, godfather, and summary statistics are consistent (directed unipartite)", {
  set.seed(1)
  simtest(20, 10, 10, 10, TRUE, FALSE)
})

test_that("simulate, godfather, and summary statistics are consistent (undirected bipartite)", {
  set.seed(1)
