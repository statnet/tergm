...
test_that("simulate.networkDynamic encodes net.obs.period correctly", {
  ...
  expect_true(all(unlist((dynsim%n%'net.obs.period')$observations)==c(0,1,1,11)),
  ...
  expect_true(all(unlist((dynsim2%n%'net.obs.period')$observations)==c(0,1,1,11,11,21)),
...
test_that("simulate.networkDynamic encodes net.obs.period correctly when stopping and restarting", {
  ...
  expect_true(all(unlist((msm.sim%n%'net.obs.period')$observations)==c(-1,  0,  0,  1,  1,  2)),
...
test_that("simulate.networkDynamic handles vertex dynamics correctly", {
  ...
  expect_true(all(sapply(-1:2,function(t){network.size.active(msm.sim,at=t)})==c(100,95,86,86)),
              label="vertex dynamics activity in stergm sim")

  expect_true(all(network.dynamic.check(msm.sim,complete=FALSE)$dyad.checks),
...
test_that("simulate.networkDynamic respects time.start and time.offset", {
  ...
  # at this point msm.sim has observations as far as 2.  Try sampling at 2 and updating from (2,3)
  msm.sim <- simulate(msm.sim ~ Form(~edges) + Persist(~edges),
                      coef=c(coef.form,coef.diss),
  ...
  expect_lte(max(unlist((msm.sim%n%'net.obs.period')$observations)), 3,
             label="net.obs.period not updated inappropriately when time.start and time.offset was explicitly set")
  expect_true(all(get.change.times(msm.sim)==0:2),
...
test_that("simulate.networkDynamic handles time.offset=0 correctly", {
  ...
  expect_lte(length((msm.sim%n%'net.obs.period')$observations), 1,
...
test_that("simulate.networkDynamic respects externally defined vertex PID", {
  ...
  dynsim<-simulate(dynsim ~ Form(~edges) + Persist(~edges),
                   ...
                   time.slices=1,verbose=TRUE, dynamic=TRUE)
  # expect to see a pid defined, but will just be numeric values
  expect_equal(dynsim%n%'vertex.pid', "letters",
               label="simulate.networkDynamic vertex pid preserved")
  expect_true(all(is.na(dynsim%v%"tergm_pid")),
...
test_that("simulate.networkDynamic handles Inf coefficients correctly", {
  ...
  expect_error(
    simulate(fit.sim ~ Form(~ edges + offset(nodemix("loc", levels2=-c(1, 3)))) + Persist(~offset(edges)),
