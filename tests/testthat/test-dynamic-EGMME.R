...
opttest({
  test_that("dynamic EGMME fits correctly", {
    ...
    expect_equal(c(coef.form,-coef.diss),coef(dynfit),tolerance=0.01,ignore_attr=TRUE)
    ...
    expect_equal(c(coef.form,-coef.diss),coef(dynfit2),tolerance=0.01,ignore_attr=TRUE)
  })
}, "EGMME")
