...
opttest({
  ...
  do.run <- function(dir, prop.weights="default"){
    ...
    test_that(paste("CMLE/CMPLE with block-diagonal constraints on complete", netdesc, "data"), {
      set.seed(123)
      ...
      expect_lte(z.error(form.mle(y0,y1), coef(fit)[1], vcov(fit)[1,1]), tolerance)
      expect_lte(z.error(diss.mle(y0,y1), coef(fit)[2], vcov(fit)[2,2]), tolerance)
    ...
    test_that(paste("CMLE/CMPLE with block-diagonal constraints on missing", netdesc, "data"), {
      set.seed(123)
      y1<-simulate(y0~edges, constraints=~blockdiag("a"), coef=theta, control=control.simulate(MCMC.burnin=n^2*2), dynamic=FALSE)
      ...
      expect_equal(fit$estimate, "CMPLE")
      expect_lte(z.error(form.mle(y0,y1m), coef(fit)[1], vcov(fit)[1,1]), tolerance)
      expect_lte(z.error(diss.mle(y0,y1m), coef(fit)[2], vcov(fit)[2,2]), tolerance)
