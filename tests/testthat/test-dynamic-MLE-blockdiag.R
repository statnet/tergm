#  File tests/testthat/test-dynamic-MLE-blockdiag.R in package tergm, part of
...
opttest({
  ...
  do.run <- function(dir, prop.weights = "default") {
    ...
    netdesc <- if (dir) "directed" else "undirected"

    test_that(
      paste(
        "CMLE/CMPLE with block-diagonal constraints on complete",
        netdesc, "data"
      ),
      {
        set.seed(123)
        ...
        expect_lte(
          z.error(form.mle(y0, y1), coef(fit)[1], vcov(fit)[1, 1]),
          tolerance
        )
        expect_lte(
          z.error(diss.mle(y0, y1), coef(fit)[2], vcov(fit)[2, 2]),
          tolerance
        )
    ...
    test_that(
      paste(
        "CMLE/CMPLE with block-diagonal constraints on missing",
        netdesc, "data"
      ),
      {
        set.seed(123)
        ...
        expect_equal(fit$estimate, "CMPLE")
        expect_lte(
          z.error(form.mle(y0, y1m), coef(fit)[1], vcov(fit)[1, 1]),
          tolerance
        )
        expect_lte(
          z.error(diss.mle(y0, y1m), coef(fit)[2], vcov(fit)[2, 2]),
          tolerance
        )
