...
stopifnot(z.error(form.mle(y0,y1,y2), fit$coef[1], vcov(fit$fit)[1,1]) <= tolerance)
stopifnot(z.error(diss.mle(y0,y1,y2), fit$coef[2], vcov(fit$fit)[2,2]) <= tolerance)
...
stopifnot(fit$estimate=="CMPLE")
stopifnot(z.error(form.mle(y0,y1,y2m), fit$coef[1], vcov(fit$fit)[1,1]) <= tolerance)
stopifnot(z.error(diss.mle(y0,y1,y2m), fit$coef[2], vcov(fit$fit)[2,2]) <= tolerance)
