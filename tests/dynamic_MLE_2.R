...
# Force CMPLE
set.seed(543)
fit<-tergm(list(y0,y1,y2) ~ Form(~edges) + Diss(~edges), estimate="CMPLE", times=c(1,2,3))

stopifnot(fit$estimate=="CMPLE")
stopifnot(z.error(form.mle(y0,y1,y2), fit$fit$coef[1], vcov(fit$fit)[1,1]) <= tolerance)
stopifnot(z.error(diss.mle(y0,y1,y2), fit$fit$coef[2], vcov(fit$fit)[2,2]) <= tolerance)

# Autodetected CMPLE
set.seed(543)
fit<-tergm(list(y0,y1,y2) ~ Form(~edges) + Diss(~edges), estimate="CMLE", times=c(1,2,3))
...
cat("====",prop.weight,"====\n")
set.seed(543)
fit<-tergm(list(y0,y1,y2) ~ Form(~edges) + Diss(~edges), estimate="CMLE", control=control.tergm(CMLE.control=control.ergm(MCMLE.effectiveSize = NULL, MCMC.samplesize = 2*1024, MCMC.burnin=10000, MCMC.interval = 1024,force.main=TRUE, MCMC.prop.weights=prop.weight)), times=c(1,2,3))
...
# Force CMPLE
set.seed(765)
fit<-tergm(list(y0,y1,y2m) ~ Form(~edges) + Diss(~edges), estimate="CMPLE", times=c(1,2,3))

stopifnot(fit$estimate=="CMPLE")
stopifnot(z.error(form.mle(y0,y1,y2m), fit$fit$coef[1], vcov(fit$fit)[1,1]) <= tolerance)
stopifnot(z.error(diss.mle(y0,y1,y2m), fit$fit$coef[2], vcov(fit$fit)[2,2]) <= tolerance)

# Autodetected CMPLE
set.seed(765)
fit<-tergm(list(y0,y1,y2m) ~ Form(~edges) + Diss(~edges), estimate="CMLE", times=c(1,2,3))
...
cat("====",prop.weight,"====\n")
set.seed(234)
fit<-tergm(list(y0,y1,y2m) ~ Form(~edges) + Diss(~edges), estimate="CMLE", control=control.tergm(CMLE.control=control.ergm(MCMLE.effectiveSize = NULL, MCMC.samplesize = 2*1024, MCMC.burnin=10000, MCMC.interval = 1024,force.main=TRUE, MCMC.prop.weights=prop.weight)), times=c(1,2,3))
