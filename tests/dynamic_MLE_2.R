...
form.mle<-function(y0,y1,y2){
  logit((network.edgecount(y1-y0,na.omit=TRUE) +
        ...
        (network.dyadcount(y0)-network.edgecount(y0-is.na(y1))-network.naedgecount(y1) +
         network.dyadcount(y1)-network.edgecount(y1-is.na(y2))-network.naedgecount(y2)))
}
...
# Force CMPLE
set.seed(543)
fit<-stergm(list(y0,y1,y2), formation=~edges, dissolution=~edges, estimate="CMPLE")
...
# Autodetected CMPLE
set.seed(543)
fit<-stergm(list(y0,y1,y2), formation=~edges, dissolution=~edges, estimate="CMLE")
...
# Force CMLE
set.seed(543)
fit<-stergm(list(y0,y1,y2), formation=~edges, dissolution=~edges, estimate="CMLE", control=control.stergm(force.main=TRUE))
...
y2m<-network.copy(y2)
set.seed(765)
y2m[as.edgelist(simulate(y1~edges, coef=theta, control=control.simulate(MCMC.burnin=n^2*2)))]<-NA

# Force CMPLE
set.seed(765)
fit<-stergm(list(y0,y1,y2m), formation=~edges, dissolution=~edges, estimate="CMPLE")
...
# Autodetected CMPLE
set.seed(765)
fit<-stergm(list(y0,y1,y2m), formation=~edges, dissolution=~edges, estimate="CMLE")
...
# Force CMLE
set.seed(765)
fit<-stergm(list(y0,y1,y2m), formation=~edges, dissolution=~edges, estimate="CMLE", control=control.stergm(force.main=TRUE))
